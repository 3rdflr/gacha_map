...
export default async function ShopDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main className='mx-auto max-w-[820px] px-4 py-10 text-gray-800'>
      ...
      {shop.address_detail && <p className='mb-1 text-sm text-gray-500'>{shop.address_detail}</p>}
      ...
      <section className='mb-8'>
        ...
        <ul className='list-disc space-y-2 pl-5 leading-relaxed'>
          ...
          <li>매장 정보는 변동될 수 있어, 방문 전 영업 여부를 다시 한 번 확인하시는 것을 권장합니다.</li>
        </ul>
      </section>
      ...
      {nearby.length > 0 && (
        <section className='mb-8'>
          ...
          <ul className='grid grid-cols-1 gap-3 sm:grid-cols-2'>
            {nearby.map((s) => (
              <li key={s.id} className='rounded-lg border border-gray-200 p-4 hover:bg-gray-50'>
      ...
      <div className='rounded-lg border border-gray-200 bg-gray-50 p-4'>
        ...
        <ul className='space-y-1'>
          ...
          <li><Link href='/' className='text-blue-600 hover:underline'>지도에서 다른 매장 보기 →</Link></li>
        </ul>
      </div>
    </main>
  );
